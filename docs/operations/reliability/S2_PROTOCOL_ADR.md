...
### Explicit recovery dismissal

Blocked local reviews and rejected completions now have explicit, audited recovery actions instead of accumulating forever. Dismissing a blocked review archives its message and journal evidence under `discardedReviews` (merged across restores, validated on load), retires whole groups together via the retained envelope when WAL bytes are already retired, and removes WAL bytes only after the archive commits; a crash in between re-blocks for a retryable, idempotent dismissal. Dismissing a rejected completion releases its reservations so held successors and paused pull pages proceed, while the admission, request and rejected receipt remain as audit, restore validation accepts dismissed identities, and the identity can never be re-admitted. Pending and accepted completions refuse dismissal. The status panel lists blocked reviews with two-tap confirm, refreshes optimistically from the dismissal result, and invalidates in-flight snapshot reads through the shared revision guard. Divergent legacy ambiguity import, nonempty-restore reconciliation and native/macOS recovery UX remain open S2 work.
...
### Native completion request and receipt integration

Completion participates in the existing ordered native action journal using its dedicated protocol type. Dependency resolution requires exact retained predecessor receipts; only pre-attempt bases can be assigned. Original request reconstruction must not normalize or truncate captured predecessor fields. The resulting wire bytes remain immutable. Large bodies use the existing staged manifest/chunk protocol and completion endpoints.
