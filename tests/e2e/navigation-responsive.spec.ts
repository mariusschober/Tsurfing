...
test('short landscapes, long text, enlarged text and reduced motion keep panels usable', async ({ page }) => {
  ...
  await page.evaluate(account => (window as any).__s1RenderAccount(account), longAccount);
...
test('primary destinations, Insights/Done and overdue navigation restrictions remain intact', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    await (window as any).__s1Storage.set('tasks', account, [{ id: 'overdue-navigation', title: 'Overdue task', completed: false, isFrog: false, createdAt: Date.now(), hashtags: [], dateAssigned }], 'cloud');
    (window as any).__s1RenderAccount(account);
  });
...
test('Plan and Current share mode; cancelling check-in and using Manual retain existing behavior', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    await (window as any).__s1Storage.set('circadian', account, { lastCheckIn, score: 90, mode: 'apex', metrics: { sunrise: false, sleepHours: 8, energy: 90, clarity: 90, interest: 90 } }, 'cloud');
    (window as any).__s1RenderAccount(account);
  });
...
test('opening Menu and crossing breakpoints preserve the active task and focus session', async ({ page }, testInfo) => {
  ...
  const focus = () => page.evaluate(async account => {
    await (window as any).__s1Storage.flushPendingLocalChanges(account);
    return (await (window as any).__s1Storage.get('tracking', account)).focusSession;
  }, user);
