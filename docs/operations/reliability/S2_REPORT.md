...
## Native revision basis checkpoint `8f2cd1621ecc63418f8cf6be56658b3b598c4ca2`
...
## Web non-focus task completion checkpoint

`8d7420ea92e25eef3ca62158e438e095b72ed44a`: **PASS_LOCAL**. Fenced Web completion without an active focus session no longer derives its task/statistics/goal/habit/event effects from a potentially stale React snapshot. `admitLocalTaskCompletion` reads the current collections inside one causal business transaction, derives effects with the existing `deriveTaskCompletion` boundary, and commits collections, ordinary compatible mutations, journal evidence and generation together. Members travel the exact existing ordinary receipt contract with `dependsOnMutationId` chains; focus-completion v2 reservations and pull-pausing are deliberately untouched. Retries reuse the original action identity; a second completion of the same task fails closed; an injected final-write failure rolls back every effect and retries cleanly. Unfenced legacy behavior is unchanged.

The rendered overdue "Mark complete" path routes through this admission when the account is fenced. A new Chromium/WebKit journey fences an account with two overdue tasks and completes both rapidly: both tasks complete, statistics reach 2, two task events persist, two `task-completion` admissions retire their journal evidence, and no blocked entry appears.

Validation: `npm run verify:release` exit 0, 724 tests in 106 files. Focused unit suite: 5 passed. `test:s1:storage` 38 passed ×3. S1 browser 42 passed; S2 browser 26 passed (13 journeys × Chromium/WebKit); full E2E 92 passed. Both real PostgreSQL matrices passed. Android: 220 tests, 0 failures, 1 hosted skip; lint/debug build passed. macOS: 226 tests, 1 hosted skip, 0 failures. Identifiers 24, migrations 32, migration hashes 32, Room hashes 9 passed. Two intermediate browser-fixture failures are retained in the working notes (planning-gate navigation requires seeded counter baselines; the gate button detaches once PlanningView renders); the final journey passes on both engines.

Remaining: native UI admission/enrollment activation, rejected/conflicting completion and legacy ambiguity recovery, macOS causal integration and full cross-client acceptance. S2 remains incomplete and S3 is not permitted. No migration, deployment, installation or live data writes occurred.

## Native UI admission checkpoint

`358896a2b030be0e2825fcd34965a5898990cbd4`: **PASS_LOCAL**. The normal Android ViewModel now admits focus intents through the private journal when one exists and keeps the legacy Room paths otherwise. Start captures target task, fresh session identity and planned duration before launching work; pause/resume/stop/extend carry the rendered session identity with an explicit kind; completion carries final details. Rejected outcomes fail closed as UI errors instead of reporting false success; completion still requires the enrolled epoch. Journals are never created implicitly, and task-only completion stays on the legacy path. Wire timestamps use millis-precision formatting so the receipt identity validator accepts them.

Eight new regression tests: repository routing proves journal detection, serial composition with stale-target rejection, fail-closed admission without a journal alongside working legacy paths, and enrollment-gated completion awarded exactly once. ViewModel tests prove legacy start without a journal, causal start/pause/extend composition from rendered state, and error surfacing with the task kept open when no session is ready.

Validation: full native gate exit 0 — 228 tests, 0 failures, one hosted-transport skip; lint and debug APK build passed. Web release verification (724 tests), S1 storage/browser journeys and PostgreSQL matrices were proven for the unchanged Web/tree state in the preceding checkpoint and rerun where affected; this slice changes Android sources only. The ViewModel test recipe is documented in-code: Main must be a real single thread (never virtual-time dispatchers, whose ticker interplay spins runTest forever), no virtual-time timeouts for real-thread Room waits (they never fire), and no class-level Main setup under Robolectric.

Remaining: Android task-only completion admission, rejected/conflicting completion and legacy ambiguity recovery, macOS causal integration and full cross-client acceptance. S2 remains incomplete and S3 is not permitted. No migration, deployment, installation or live data writes occurred.

## Explicit recovery dismissal checkpoint

`b37c0d2e91bcd01e5c5c906ec3f473d626fe77ff`: **PASS_LOCAL**. Blocked local reviews (`STALE_GROUP_INTENT`, `STALE_FOCUS_INTENT`, `CAUSAL_CAPTURE_REVIEW`, fallback reviews) can now be dismissed after explicit review. The blocked message and journal evidence move to a retained `discardedReviews` archive (merged across restores, validated on load); whole groups retire together, resolving membership through the retained envelope when WAL bytes are already retired; WAL bytes are removed only after the archive commits, and a crash in between re-blocks for a retryable dismissal. Committed projections, outbox, receipts and conflicts are untouched. Dismissal is idempotent and refuses unknown, live or malformed requests.

Rejected causal completions dismiss with their reservations released, so held successors and paused pull pages proceed; admissions, requests and the rejected receipt remain as audit, restore validation accepts dismissed identities, and the identity can never be re-admitted (prepare/commit/retry all fail closed). Pending and accepted completions refuse dismissal. SyncStatus lists blocked reviews with two-tap confirm, refreshes optimistically from the dismissal result, and invalidates in-flight snapshot reads through the shared revision guard so a pre-dismissal read cannot repaint stale reviews.
...
## Native task-only completion checkpoint

`2bf78ded9ec209da925d880d47564abe81da55c0`: **PASS_LOCAL**. Prepared Android accounts admit task-only completions through the private journal. `NativeCausalStore.admitTaskCompletion` validates intent (millis-precision timestamps, day/zone/duration/flow ranges), guards the action identity across focus/counter/day/completion/task namespaces with duplicate-first ordering, then derives task/statistics/goal/habit/event effects from current collections inside the same Room transaction through the existing atomic completion path. Members keep the exact ordinary receipt contract with `dependsOnMutationId` chains; retries reuse the original admission; an already-completed task and mid-transaction failures roll back everything. The local generation sequence stays with timeline admissions (a task-only admission consuming a sequence number fails timeline validation; verified by a failing-then-passing assertion during development). The normal ViewModel routes task completion through the admission with per-tap identity retention when a journal exists; legacy paths are unchanged without one.
...
## macOS focus admission checkpoint

`bc58228c985fc8708fc3335ac6fd92d9cdfed63f`: **PASS_LOCAL**. The macOS journal admits focus commands through the real `CausalFocus` domain: validation, authoritative parent-revision/epoch derivation, task-eligibility check, atomic journal/projection/outbox/generation commit, and replay validation on every load. Duplicates return stored outcomes; stale/terminal commands record rejected outcomes without touching projections; closed tasks, bare completion (reserved for the atomic task coordinator) and malformed intents fail closed with the journal unchanged. Cross-kind action identity is guarded. Four admission tests cover serial composition with duplicate and stale/terminal behavior, idempotent retry, fail-closed intents, and identity collision.

Validation: `xcodebuild test` exit 0 — 237 tests, 0 failures, one hosted-transport skip; source-membership gate passes. Web release verification exit 0 (732 tests, 108 files; no Web sources changed). Counter/day admission, transport, receipts, history, enrollment, completion and UI activation remain. No migration, deployment, installation or live data writes occurred.

## macOS counter/day admission checkpoint

`ab3989f74d6d2913684f623c1dc0452d81873172`: **PASS_LOCAL**. The macOS journal admits day selections and counter increments: `establish` records current tracking counts as the day baseline (a second baseline fails closed); `select` records intent without projecting; increments project atomically when their day baseline exists and retain with `baselinePending` otherwise, never inventing a zero baseline. Validation now replays focus, counter and day admissions over one shared generation sequence and projects counters per established day through the shared `CounterLedger`. Three admission tests cover composition with idempotent retries, unknown-day retention without touching yesterday, fail-closed validation, and cross-kind identity collision through a real prior admission.

Validation: `xcodebuild test` exit 0 — 240 tests, 0 failures, one hosted-transport skip; source-membership gate passes. Web release verification exit 0 (732 tests, 108 files; no Web sources changed). Transport, receipts, history, enrollment, completion and UI activation remain. No migration, deployment, installation or live data writes occurred.

## macOS causal transport checkpoint

`2fb28d3605ddf60cf266aa4dc8783aaf80628c10`: **PASS_LOCAL**. macOS sends saved version-2 focus/counter/day operations unchanged with 256 KiB request and 8 MiB receipt bounds, classifying retryable (408/425/429/5xx) versus review (409/other) failures without reflecting upstream diagnostics. Receipts validate exactly against the saved operation and account scope, including all shared fixture vectors with tamper rejection; completion receipts are explicitly deferred to the atomic validator. Saved requests are immutable; archival and accepted-outbox retirement commit together while rejected receipts retain their pending intent. Transport success alone retires nothing. Five tests cover fixture vectors, immutable requests, atomic retirement, rejected retention, missing-request fail-closed, and bounds/classification.

Validation: `xcodebuild test` exit 0 — 245 tests, 0 failures, one hosted-transport skip; source-membership gate passes. Web release verification exit 0 (732 tests, 108 files; no Web sources changed). Identifiers 24 pass. History, enrollment, completion and UI activation remain. No migration, deployment, installation or live data writes occurred.
