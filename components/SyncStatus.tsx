...
export const SyncStatus: React.FC<{ userKey: string; closeSignal?: string; onOpenChange?: (open: boolean) => void }> = ({ userKey, closeSignal, onOpenChange }) => {
  ...
  return (
    <div className="min-w-0">
      <button ref={buttonRef} type="button" onClick={() => setOpen(value => !value)} aria-label={`Sync status: ${labels[status.state]}`} aria-expanded={open} aria-controls={panelId} aria-haspopup="dialog" className="header-control sync-status-control text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-slate-700" title={status.message || (status.lastSuccessfulSync ? `Last synced ${new Date(status.lastSuccessfulSync).toLocaleString()}` : labels[status.state])}>
